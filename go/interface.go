...
type Store interface {
	...
	// List returns every job, oldest first, AND, as *ErrUnreadable, the ones it
	// could not decode. Both at once: a sweep that returned only the error would
	// stop a supervisor doing the work it can still do, and one that returned
	// only the records would let it report a directory it cannot read as empty.
	List() ([]*Record, error)
	...
	//
	// Reports *ErrUnreadable beside its answer, for List's reason: "no orphans"
	// and "I could not read six records" are different answers and a sweeper
	// that cannot tell them apart calls itself healthy while work is stranded.
	...
	//
	// mutate is applied to the record as it stands at the moment of the write,
	// never to a copy read earlier. The epoch does not give a binding this for
	// free: it fences a lapsed holder and does not move when its holder writes,
	// so two writers under one lease present the same valid epoch. A binding
	// that cannot run mutate at the store must make its write conditional on
	// the record it read and re-apply mutate when that condition fails.
