...
// [JOB-E6] governs a value this layer spells for itself. [JOB-E7] governs one it
// read and is only carrying, and round_trip is the only view of that one.
static std::string written(const std::string& value) { return Value(value).dump(); }
...
static void test_strings() {
    ...
    check("a solidus escape decodes, and the spelling it arrived in is carried",
          Value::parse("\"a\\/b\"").get<std::string>() == "a/b" &&
              round_trip("\"a\\/b\"") == "\"a\\/b\"");
...
static void test_what_a_document_may_not_be() {
    ...
    // [JOB-E9] draws the line at the decoded name, and both sides of it are the
    // rule. Widening it would refuse documents nothing in the contract refuses;
    // narrowing it lets one name in through a second spelling.
    check("two spellings of one name are one name",
          refused("{\"x\":1,\"\\u0078\":2}"));
    check("a repeated name three levels down is refused",
          refused("{\"a\":{\"b\":[{\"c\":1,\"c\":2}]}}"));
    check("the same name in two sibling objects is not a repeat",
          !refused("{\"o\":{\"a\":1},\"p\":{\"a\":2}}"));
    check("a name repeated in a parent and its child is not a repeat",
          !refused("{\"a\":{\"a\":1}}"));
    check("the comparison is case-sensitive", !refused("{\"a\":1,\"A\":2}"));
    check("the comparison does not normalize",
          !refused("{\"\\u00e9\":1,\"e\\u0301\":2}"));
...
// [JOB-E6] in full, on the writer three implementations are compared byte for
// byte against. Every line here was a disagreement rather than a rule until the
// policy was declared: Go escaped & < > and U+2028, Python escaped everything
// above ASCII, this writer escaped none of them, and the first record carrying
// a query string or an accented filename would have settled it by breaking.
static void test_the_escape_policy() {
    check("an ampersand is raw", written("a&b") == "\"a&b\"");
    check("angle brackets are raw", written("<x>") == "\"<x>\"");
    check("a solidus is raw and never escaped", written("/") == "\"/\"");
    check("e-acute is raw UTF-8", written("\xc3\xa9") == "\"\xc3\xa9\"");
    check("an astral character is raw UTF-8, never a surrogate pair",
          written("\xf0\x9f\x98\x80") == "\"\xf0\x9f\x98\x80\"");
    check("DEL is raw, because JSON requires escaping only below 0x20",
          written("\x7f") == "\"\x7f\"");
    check("U+2028 is escaped", written("\xe2\x80\xa8") == "\"\\u2028\"");
    check("U+2029 is escaped", written("\xe2\x80\xa9") == "\"\\u2029\"");
    check("a control character with a two-character escape takes it",
          written("\t") == "\"\\t\"");
    check("every other control character is \\u00xx in lower-case hex",
          written("\x1f") == "\"\\u001f\"");

    Value built = Value::object();
    built["p"] = std::string("a\xff" "b");
    check("a byte that is not well-formed UTF-8 is written as the escape, not as the glyph",
          built.dump() == "{\"p\":\"a\\ufffdb\"}");
}

// [JOB-E7]. A spec and a checkpoint are opaque, so their author's spelling is
// what a reader downstream must see -- not this writer's idea of it. The
// contract page said for days that a parsed representation could not do this.
static void test_carrying_a_payload() {
    const std::string hostile =
        "{\"url\":\"https://x/A\\/b\",\"ratio\":1.50,\"exp\":1e2,\"zed\":-0.0,"
        "\"big\":12345678901234567890,\"od\\/d\":1,\"esc\":\"\\ud83d\\ude00\","
        "\"zebra\":1,\"apple\":2}";
    check("every byte of an opaque payload survives being carried",
          round_trip(hostile) == hostile);
    check("a key's spelling is carried too",
          round_trip("{\"a\\/b\":1}") == "{\"a\\/b\":1}");
    check("a payload read twice does not drift",
          round_trip(round_trip(hostile)) == hostile);

    // The one place [JOB-E6] outranks [JOB-E7]: Go and Python reach these two
    // by rewriting the finished document, so a span kept here must not become
    // the only copy where the pair survives raw.
    check("a raw U+2028 inside a payload is still escaped",
          round_trip("{\"a\":\"p\xe2\x80\xa8q\"}") == "{\"a\":\"p\\u2028q\"}");
}
...
int main() {
    ...
    test_the_accessors();
    test_the_escape_policy();
    test_carrying_a_payload();
