...
std::int64_t micros_of(TimePoint::duration d) {
    return std::chrono::duration_cast<std::chrono::microseconds>(d).count();
}

// A record's year is four digits, so this is every instant one can spell.
constexpr std::int64_t kWireEarliest = -62167219200000000;
constexpr std::int64_t kWireLatest = 253402300799999999;

bool wire_spells(std::int64_t micros) {
    return micros >= kWireEarliest && micros <= kWireLatest;
}

// The span this build's clock holds is not the span the wire permits, and which
// is wider depends on the tick: libstdc++ counts nanoseconds in an int64 and
// reaches 1677 to 2262, MSVC counts 100 ns ticks and reaches ±29,227 years. An
// instant outside the overlap is refused at whichever end it falls, because the
// alternative is an int64 wrapping and year one arriving as 1754.
bool clock_holds(std::int64_t micros) {
    return micros >= micros_of(TimePoint::duration::min()) &&
           micros <= micros_of(TimePoint::duration::max());
}
...
std::string format_rfc3339(TimePoint t) {
    const std::int64_t micros = micros_of(t.time_since_epoch());
    if (!wire_spells(micros)) {
        throw Invalid("bad_timestamp: an instant outside the four-digit year a record spells");
    }
...
TimePoint parse_rfc3339(const std::string& raw) {
    ...
    const std::int64_t secs =
        ...
        offset_seconds;
    const std::int64_t micros = secs * 1000000 + frac_micros;
    if (!wire_spells(micros)) {
        throw Invalid("bad_timestamp: \"" + raw + "\" is not an instant a record can spell");
    }
    if (!clock_holds(micros)) {
        throw Invalid("bad_timestamp: \"" + raw + "\" is outside the range this build's clock holds");
    }
    return TimePoint(
        std::chrono::duration_cast<TimePoint::duration>(std::chrono::microseconds(micros)));
}
...
std::string Record::encode() const {
    ...
    if (envelope) {
        Json ev = Json::object();
        ev["schema"] = envelope->schema;
        if (!envelope->actions.empty()) {
            ev["actions"] = envelope->actions;
        }
        d["envelope"] = std::move(ev);
    }
    d["state"] = state;
    d["spec"] = spec;
    if (canonical) {
    ...
    if (!extensions.empty()) {
        // A Value keeps insertion order and the other two implementations sort;
        // the record is compared byte for byte, so this sorts too.
...
// The rules that read `critical`, in the one order [JOB-D10] fixes them in.
//
...
std::vector<std::string> read_critical(const std::vector<std::string>& critical,
    ...
    for (const std::string& name : critical) {
        ...
        if (!is_known_feature(name)) {
            throw UnknownSchema("this record requires \"" + name +
                                "\", which this implementation cannot read");
        }
        if (std::find(content.begin(), content.end(), name) == content.end()) {
            throw Invalid("critical names \"" + name +
                          "\" and content does not carry it -- a declaration about nothing");
        }
        kept.push_back(name);
    }
    return kept;
...
std::vector<std::string> name_list(const Json& d, const char* where) {
    if (!d.contains(where) || d.at(where).is_null()) {
        return {};
    }
    try {
        return d.at(where).get<std::vector<std::string>>();
    } catch (const Json::Error&) {
        throw Invalid(std::string(where) + " must be an array of strings");
    }
}

Record decode_object(const Json& d) {
    reject_unknown_keys(d,
                        {"schema", "content", "critical", "id", "kind", "envelope", "state", "spec",
                         "checkpoint", "progress", "lease", "delegation", "requires", "error",
                         "intent", "extensions", "created_at", "updated_at"},
                        "record");

    Record r;
    r.content = name_list(d, "content");
    r.critical = name_list(d, "critical");
    ...
    r.critical = read_critical(r.critical, r.content);
    ...
    if (d.contains("envelope") && !d.at("envelope").is_null()) {
        const Json& ev = d.at("envelope");
        if (!ev.is_object()) {
            throw Invalid("envelope must be an object");
        }
        reject_unknown_keys(ev, {"schema", "actions"}, "envelope");
        Envelope e;
        e.schema = string_field(ev, "schema");
        e.actions = name_list(ev, "actions");
        r.envelope = std::move(e);
    }
...
Record Record::decode(const std::string& text) {
    ...
    try {
        ...
        // A Json::Error is not a JobError, so a caller catching this layer's own
        // refusals does not catch it and the process ends on a record somebody
        // else wrote. An integer past 64 bits reached here.
        throw Invalid(std::string("a value this record cannot hold: ") + e.what());
    }
}
...
bool is_known_feature(const std::string& name) {
    return name == feature::kBase || name == feature::kIntent || name == feature::kDelegation ||
           ...
           name == feature::kRanges || name == feature::kEnvelope;
}
...
std::vector<std::string> known_features() {
    std::vector<std::string> names = {feature::kBase,     feature::kIntent, feature::kDelegation,
                                      ...
                                      feature::kRanges,   feature::kEnvelope};
    std::sort(names.begin(), names.end());
    return names;
}
...
constexpr std::size_t kMaxSchema = 128;
constexpr std::size_t kMaxLabel = 63;
constexpr std::size_t kMaxAction = 64;

bool lower_alnum(char c) { return (c >= 'a' && c <= 'z') || (c >= '0' && c <= '9'); }

// One part of a name: lower-case letters, digits and interior separators.
// Deliberately the smallest alphabet that can name anything, because everything
// it leaves out is what a URL, a path and a command line are made of.
bool is_label(const std::string& s, const char* extra, std::size_t limit) {
    if (s.empty() || s.size() > limit) {
        return false;
    }
    if (!lower_alnum(s.front()) || !lower_alnum(s.back())) {
        return false;
    }
    for (const char c : s) {
        if (!lower_alnum(c) && std::string(extra).find(c) == std::string::npos) {
            return false;
        }
    }
    return true;
...
bool is_base_action(const std::string& bare) {
    return bare == action::kPause || bare == action::kResume || bare == action::kCancel ||
           bare == action::kRecall;
}

bool valid_schema(const std::string& s) {
    if (s.empty() || s.size() > kMaxSchema) {
        return false;
    }
    if (count_of(s, '/') != 1 || count_of(s, '@') != 1) {
        return false;
    }
    ...
    for (std::size_t start = 0;;) {
        const std::size_t dot = ns.find('.', start);
        const std::string part =
            dot == std::string::npos ? ns.substr(start) : ns.substr(start, dot - start);
        if (!is_label(part, "-", kMaxLabel)) {
            return false;
        }
    ...
    if (!is_label(s.substr(slash + 1, at - slash - 1), "-", kMaxLabel)) {
        return false;
    }
    const std::string version = s.substr(at + 1);
    if (version.empty() || version.size() > 9 || version[0] < '1' || version[0] > '9') {
        return false;
    }
...
bool split_action(const std::string& name, std::string* out_schema, std::string* out_bare) {
    const std::size_t hashes = count_of(name, '#');
    if (hashes > 1) {
        return false;
    }
    if (hashes == 0) {
        ...
        return is_label(name, "-_", kMaxAction);
    }
    const std::size_t hash = name.find('#');
    const std::string schema = name.substr(0, hash);
    const std::string bare = name.substr(hash + 1);
    if (out_schema != nullptr) {
        *out_schema = schema;
    }
    if (out_bare != nullptr) {
    ...
    return valid_schema(schema) && is_label(bare, "-_", kMaxAction);
}

void Envelope::validate() const {
    if (!valid_schema(schema)) {
        throw Invalid("\"" + schema +
                      "\" is not a schema identifier; a schema is named, never fetched");
    }
    for (std::size_t i = 0; i < actions.size(); ++i) {
        const std::string& name = actions[i];
        const auto upto = actions.begin() + static_cast<std::ptrdiff_t>(i);
        if (std::find(actions.begin(), upto, name) != upto) {
            throw Invalid("action \"" + name + "\" is declared twice");
        }
        std::string declared;
        std::string bare;
        if (!split_action(name, &declared, &bare)) {
            throw Invalid("\"" + name + "\" is not an action name");
        }
        if (declared.empty()) {
            ...
            if (!is_base_action(bare)) {
                throw Invalid("\"" + name +
                              "\" is bare and is not one of this layer's actions; a kind's own "
                              "action carries the schema that declares it");
            }
        } else if (declared == kBaseSchema) {
            // One spelling of one action, so that supports() is a comparison and
            // not a negotiation.
            throw Invalid("\"" + name + "\" spells a base action the long way; write it bare");
            ...
            // claim to, and a supervisor matching on the name alone would act on
            // somebody else's authority.
            throw Invalid("action \"" + name +
                          "\" names a schema this record does not declare (\"" + schema + "\")");
        }
    }
}
...
void envelope_unmoved(const std::optional<Envelope>& before,
    ...
    if (before.has_value() == after.has_value() && (!before.has_value() || *before == *after)) {
        return;
    }
    throw Invalid("the envelope is a property of the kind and is written once, at submit");
}

bool Record::supports(const std::string& a) const {
    if (!envelope) {
        return false;
    }
    return std::find(envelope->actions.begin(), envelope->actions.end(), a) !=
           envelope->actions.end();
}

void Record::ask(const std::string& a, const Supervisor& supervisor) const {
    std::string declared;
    std::string bare;
    if (!split_action(a, &declared, &bare)) {
        throw Invalid("\"" + a + "\" is not an action name");
    }
    if (!envelope) {
        throw NotSupported(id + " declares no envelope, so it declares no actions");
    }
    const auto holds = [&supervisor](const std::string& s) {
        return std::find(supervisor.schemas.begin(), supervisor.schemas.end(), s) !=
               supervisor.schemas.end();
    };
    if (!holds(envelope->schema)) {
        throw UnknownSchema("\"" + envelope->schema +
                            "\", which this supervisor was not written against");
    }
    if (!declared.empty() && !holds(declared)) {
        throw UnknownSchema("\"" + declared + "\", which this supervisor was not written against");
    }
    if (!supports(a)) {
        throw NotSupported(kind + " does not declare \"" + a + "\"");
    }
    if (std::find(supervisor.actions.begin(), supervisor.actions.end(), a) ==
        supervisor.actions.end()) {
        throw NotSupported("this supervisor does not implement \"" + a + "\"");
    }
}
...
void derive_features(const Record& r, std::vector<std::string>& out_content,
    ...
    if (r.envelope.has_value()) {
        out_content.push_back(feature::kEnvelope);
        out_critical.push_back(feature::kEnvelope);
    }
