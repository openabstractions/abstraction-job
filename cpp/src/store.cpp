...
Record FileStore::update(const std::string& id, std::int64_t epoch,
    ...
    return change(id, [&](Record& r) {
        ...
        // [JOB-V4]: the envelope is a property of the KIND, so a lease holder
        // cannot move it. The service binding gets this for free — it enumerates
        // the fields a write may carry — but this binding runs the caller's own
        // callable against the record, so here the rule is checked rather than
        // arranged.
        const std::optional<Envelope> kind_says = r.envelope;
        mutate(r);
        envelope_unmoved(kind_says, r.envelope);
        r.updated_at = now();
    });
}
