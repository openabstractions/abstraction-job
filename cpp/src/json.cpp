...
// string literal before ES2019, and they are the only two characters where the
// formats disagree. [JOB-E6] escapes them so one record reads as both.
bool line_separator(const std::string& s, std::size_t at, std::size_t run) {
    return run == 3 && static_cast<unsigned char>(s[at]) == 0xE2 &&
           static_cast<unsigned char>(s[at + 1]) == 0x80 &&
           (static_cast<unsigned char>(s[at + 2]) == 0xA8 ||
            static_cast<unsigned char>(s[at + 2]) == 0xA9);
}
...
void escape_into(std::string& out, const std::string& s) {
    ...
    while (i < s.size()) {
        ...
        if (c >= 0x80) {
            ...
            // parser then refuses to read back. [JOB-E6] spells that
            // substitution as an escape, so a person reading the record sees
            // that a byte was lost where a raw U+FFFD glyph reads as content.
            const std::size_t run = utf8_run(s, i);
            if (run == 0) {
                out += "\\ufffd";
                ++i;
            } else if (line_separator(s, i, run)) {
...
// [JOB-E7] hands an opaque value's spelling back unchanged and [JOB-E6] escapes
// U+2028 and U+2029 wherever they stand; the two meet only on a span that holds
// one raw, and there the escape wins. Go and Python reach the same bytes by
// rewriting the finished document, so a span kept here must not be the one place
// the pair survives.
bool holds_line_separator(const std::string& span) {
    return span.find("\xe2\x80\xa8") != std::string::npos ||
           span.find("\xe2\x80\xa9") != std::string::npos;
}
...
struct Reader {
    ...
    Value read_value(int depth) {
        ...
        switch (peek()) {
            ...
            case '"': {
                ...
                Value v(read_string());
                v.spelling_ = spelling_from(start);
                return v;
            }
    ...
    Value read_object(int depth) {
        ...
        for (;;) {
            skip_space();
            const std::size_t key_start = at;
            std::string key = read_string();
            std::string key_spelling = spelling_from(key_start);
            skip_space();
            if (peek() != ':') {
            ...
            out.key_spellings_.push_back(std::move(key_spelling));
    ...
    std::string spelling_from(std::size_t start) const {
        std::string span = s.substr(start, at - start);
        return holds_line_separator(span) ? std::string() : span;
    }
...
void Value::erase(const std::string& key) {
    ...
    for (std::size_t i = 0; i < keys_.size(); ++i) {
        if (keys_[i] == key) {
            ...
            if (i < key_spellings_.size()) {
                key_spellings_.erase(key_spellings_.begin() + static_cast<std::ptrdiff_t>(i));
            }
            return;
        }
    }
...
void Value::write(std::string& out, int indent, int level) const {
    ...
    for (std::size_t i = 0; i < items_.size(); ++i) {
        ...
        out += inner;
        if (object) {
            if (i < key_spellings_.size() && !key_spellings_[i].empty()) {
                out += key_spellings_[i];
            } else {
