...
// below.
//
// Every scalar read from a document is kept as the literal text it arrived as
// and written back unchanged — a number's digits and a string's escapes alike.
// `spec`, `checkpoint` and `extensions` are opaque to us, so their spelling is
// somebody else's; parsing 1.50 into a double and printing 1.5, or reading
// "a\/b" and writing "a/b", is how one value comes to have two spellings in a
// record three implementations compare byte for byte [JOB-E7]. A Value built
// from a C++ string has no such spelling and is escaped by [JOB-E6], which is
// what keeps the record's own fields canonical however they arrived.
...
private:
    ...
    bool boolean_ = false;
    std::string text_;
    // A parsed string's source span, quotes included, so it can never be empty
    // and empty means "built here, spell it by [JOB-E6]". key_spellings_ says
    // the same for the keys, and runs short of keys_ once operator[] has added
    // one, which is why every read of it is bounds-checked rather than indexed.
    std::string spelling_;
    std::vector<std::string> keys_;
    std::vector<std::string> key_spellings_;
